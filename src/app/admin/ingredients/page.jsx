...
export default function IngredientsTablePage() {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
            filteredIngredients = filteredIngredients.filter((ingredient) =>
                Array.from(statusFilter).includes(ingredient.status),
            );
        }

        return filteredIngredients;
    }, [ingredients, filterValue, statusFilter]);
    ...
    const renderCell = useCallback((ingredient, columnKey) => {
        ...
        switch (columnKey) {
            ...
            case "status":
                return (
                    <Chip className="capitalize" color={statusColorMap[ingredient.status]} size="sm" variant="flat">
                        {cellValue}
                    </Chip>
                );
            case "actions":
        ...
        return (
            <div className="flex flex-col gap-4 px-6 py-6">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-small">Total {ingredients.length} ingredientes</span>
